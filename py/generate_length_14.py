    ...
    for op1, op2, op3 in op_combinations:
        for base, exp, power_str, _ in POWERS:
            ...
            while attempts < max_attempts and len(formulas) < TARGET_COUNT * 3:
                ...
                d = random.randint(3, 199)
                e = random.randint(3, 99)
                ...
                # 检查长度限制
                if len(a_str) > 3 or len(d_str) > 3 or len(e_str) > 2:
                    continue
                ...
                if result is not None and result >= 0:
                    ...
                    if len(eq) == TARGET_LENGTH:
                        if not eq.split('=')[0].startswith(('2**', '3**', '4**', '5**', '10**')):
                            formulas.add(eq)
        ...
        for op1, op2, op3 in op_combinations:
            for base, exp, power_str, _ in POWERS:
                ...
                while attempts < max_attempts and len(formulas) < TARGET_COUNT * 3:
                    ...
                    if len(a_str) > 3 or len(b_str) > 3 or len(e_str) > 2:
                        continue
