const game = await import('../utils/games/math.js').then(m => m.default || m);
...
export class Math extends plugin {
  constructor() {
    super({
      ...
      rule: [
        {
          reg: /^#(?:math|Math|数学|公式).*(排行榜|榜|leaderboard|rank).*$/i,
          fnc: 'showLeaderboard'
        },
        {
          reg: /^#(?:math|Math|数学|公式)(.*)$/i,
          fnc: 'math'
        },
        {
          reg: /^[#!！][0-9+\-*/\*\*=]+$/,
  ...
  async showLeaderboard(e) {
    ...
    if (!isGlobal && !groupId) {
      await e.reply('群排行榜功能仅支持群聊使用，请使用"#math总排行榜"查看全局排行榜。');
      return true;
    }
    ...
    if (!winsTop.length && !gamesTop.length && !rateTop.length) {
      const emptyMsg = isGlobal 
        ? '全局还没有任何 数学 战绩，快来开一局吧！'
        : '当前群聊还没有任何 数学 战绩，快来开一局吧！';
    ...
    const title = isGlobal ? '🌍 数学 全局排行榜' : '📊 数学 群排行榜';
