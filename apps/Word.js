const game = await import('../utils/games/word.js').then(m => m.default || m);
...
export class Word extends plugin {
  constructor() {
    super({
      ...
      rule: [
        {
          reg: /^#[Ww]ord.*(排行榜|榜|leaderboard|rank).*$/i,
          fnc: 'showLeaderboard'
        },
        {
          reg: /^#[Ww]ord(.*)$/i,
...
${definition}`);
  ...
  async showLeaderboard(e) {
    ...
    if (!isGlobal && !groupId) {
      await e.reply('群排行榜功能仅支持群聊使用，请使用"#word总排行榜"查看全局排行榜。');
      return true;
    }
    ...
    if (!winsTop.length && !gamesTop.length && !rateTop.length) {
      const emptyMsg = isGlobal 
        ? '全局还没有任何 单词 战绩，快来开一局吧！'
        : '当前群聊还没有任何 单词 战绩，快来开一局吧！';
    ...
    const title = isGlobal ? '🌍 单词 全局排行榜' : '📊 单词 群排行榜';
